...
while IFS= read -r dir; do
    ...
    # Public packages (pkg/v1/**, pkg/v2/**, …) additionally require README.md.
    # The pkg/ root itself and pkg/<major>/ index are exempt because they only
    # carry CLAUDE.md indices, not Go code; the loop already filtered to dirs
    # with .go files, so this exemption is implicit.
    case "$dir" in
        pkg/v[0-9]*/*)
...
if [ "$missing_any" -ne 0 ] || [ "$missing_pub_readme" -ne 0 ]; then
    echo ""
    echo "Pre-commit: package documentation policy violated."
    echo "  - internal/* packages must have CLAUDE.md OR README.md."
    echo "  - pkg/v*/** public packages must have README.md (CLAUDE.md additionally if relevant)."
