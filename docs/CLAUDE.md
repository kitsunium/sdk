...
| `adr/0027-sdk-metrics-domain.md` | Observability domain (`metrics`): 9th core sibling, Counter/Gauge/Histogram + `Meter` + `Exporter` registry; in-memory meter + the stdlib `text` and `prometheus` exporters; error blocks `0.2.9.*` + `0.3.45.*`; labels and the Prometheus text exposition format shipped in place, OTLP + Prometheus protobuf still deferred | Accepted (Phase B) |
