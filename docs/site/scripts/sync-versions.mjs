...
async function materialiseRelease(major, release, sourceRoot) {
  ...
  if (existsSync(pkgMajor)) {
    ...
    for (const sub of subs) {
      ...
      let body = stripGomarkdocIndex(raw);
      //: Append the package's BENCH.md (if any) under a "## Benchmarks"
      //: H2 — same pattern across codec / errs / logger so consumers
      //: get the perf picture at the bottom of each package page
      //: without leaving the doc. Bench is regenerated by
      //: `make bench` (codec) / `go test -bench=. -benchmem` (errs,
      //: logger) and committed alongside the README.
      const benchPath = join(pkgMajor, sub.name, "BENCH.md");
      if (existsSync(benchPath)) {
        const benchRaw = await readFile(benchPath, "utf8");
        const benchBody = benchRaw
          .replace(/^<!--[^>]*-->\s*/, "") //: drop the "generated by" HTML comment
          .replace(/^#\s+.+?\n+/m, ""); //: drop the leading H1 (we re-add an H2)
        body += `\n\n## Benchmarks\n\n${benchBody}`;
      }
      await writeFile(
