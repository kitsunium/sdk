...
Note what this ADR does **not** rest on. `compute-bumps.sh:80-102` also claims to cut a release for an `internal/`-only change on its own, when `bazel query rdeps(//pkg/..., //internal/<X>/...)` is non-empty. That path was not observed working: #214 (`c707de5`) changed four non-documentation files under `internal/service/codec/ndjson/` and cut **no tag at all** — `pkg/v0.3.7` does not exist. In that release run, `Setup Bazel` took 1s and `Compute majors to bump` 31s before `Cut and push tags` was skipped, which is far too little for a cold rdeps query over this repository. Whether the query answered "no" or failed cannot be recovered, because the script discards its stderr (`bazel query … 2>/dev/null` feeding `grep -q .`), so an error and an empty result are the same value. That is a separate defect, of the same family as the ones above and worse in degree — not a wrong size but no release at all — and it is tracked on its own. This ADR is deliberately independent of it: range-sharing alone makes `internal/` changes publishable, and that is measured.
