...
- `Example_xxx` functions in `_test.go` files (gomarkdoc renders them automatically under each referent — quality-of-life addition). **Reopened 2026-09-03 as a correctness item, not a quality-of-life one.** A consumer copied the "Render safely" snippet from `pkg/v1/errs/USES.md` and was rejected by the compiler: it passed the two-value `CodeOf`/`ReasonOf` into a single-value context. An audit of all 344 fenced Go blocks under `pkg/v1` (287 gomarkdoc signature renderings, 57 real examples), cross-checked against the typed API loaded with `go/packages` for symbol existence, argument count and multi-result-in-single-value context, found one further defect — `metrics.IncCounter`, a symbol `pkg/v1/metrics` has never exported. Both are fixed, but the reason they survived is structural: **the repository contains zero `Example` functions, so no documentation example is compiled by any lane.** The examples split cleanly — every `USES.md` body (codec, errs, logger) is mechanically compilable and was compiled during the audit, with stubs only for deliberately fictitious identifiers such as `User`, `myFile`, `remoteSyslogSink`; a minority stay illustrative because they reference a caller's own domain. The work item is therefore: move the compilable ones into `Example*` functions in `*_external_test.go` so the compiler reviews them and `go test` gates them, and add a guard in the `scripts/pre-commit/` family asserting each `USES.md` Go block has a matching `Example`. Sized as its own change; deliberately not bundled into the fixes.
