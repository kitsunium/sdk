...
1. **A same-package doc link must resolve.** `tools/genindex -check-doclinks <dir>…` walks every package (production files only, `testdata`/`vendor`/`node_modules`/dot directories pruned), parses every doc comment documentation renders — package clause, top-level declarations and specs, the fields and interface methods of top-level types, exported or not; not a parameter's comment, nor a field of a struct declared inside a function body, which nothing publishes — and fails on every `[Name]`, `[Type.Member]` or `[*Type.Member]` that does not resolve, at the line that writes it. A package that does not parse fails the check; "nothing checked" is not "nothing wrong".
2. **Per platform.** A comment is judged against the symbols its package declares on each platform that compiles its file — the ten GOOS/GOARCH cells of the cross-build lane, file selection by the go command's own `go/build` matching — because pkg.go.dev renders a package per build context and a `_linux.go` file declares what a `_windows.go` one does not. One symbol table over the union of every file would pass a link to a symbol no single platform has. A link dead on only some of the platforms that compile it is reported with those platforms.
3. **A facade links the alias, and names the member after it: `[Type].Member`.** It reads the same as the link it replaces, it resolves, and on pkg.go.dev it lands on the alias, whose declaration names the aliased type where the member is documented.
4. **A wrong name is corrected to the name that resolves** — qualified through the file's own imports (`[corelock.LockBackendFailed]`, `[coreproc.Spec.Path]`), given its receiver (`[Group.Wait]`), or taken out of brackets when it was never meant as a link (a byte layout, a symbol in a package this one may not import).
5. **It is a gate.** `make doclinks` runs the check over the repository; `make lint-check`, which `scripts/ci-gates-check.sh` lists and the required `bazel` job runs, invokes it, and so does `make lint`. It needs the Go toolchain and nothing else — no network, no Bazel.
6. **In `tools/genindex`, not a new tool.** genindex already walks the repository's packages through `go/doc` to build the docs site's symbol index, it is stdlib-only and outside `go.work` (`tools/CLAUDE.md`), and a new module would add a ninth `go.mod` for one function.
