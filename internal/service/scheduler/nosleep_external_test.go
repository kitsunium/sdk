...
// make the scheduler untestable in the way ADR 0039 set out to fix: a cadence
// assertion would become a sleep, a sleep would become a tolerance, and a
// tolerance would become a flake somebody eventually deletes.
//
// Package time's READING and VALUE surface — time.Time, time.Duration,
// time.Date, time.Month, time.LoadLocation — is not listed and is used freely:
// the domain is about calendars, and a calendar needs them.
...
// central claim, and it covers the production sources AND the test suite.
//
// The claim is worth enforcing rather than asserting because it has already
// been false once: before clock.Waiter existed, "inject the clock and it
// becomes testable" was empty — there was nothing temporal to inject, so any
// timing test had to sleep. A comment saying "do not sleep here" would not
// survive the first contributor in a hurry; a named test does.
//
...
func TestPackageNeverWaitsOnTheWallClock(t *testing.T) {
	t.Parallel()
	dir := filepath.Join(sdkRoot(t), "internal", "service", "scheduler")
...
`,
		...
		{
			name: "reading and building times is not a wait",
			source: `package fixture

import "time"

func reads() time.Time {
	return time.Date(2031, time.March, 7, 0, 0, 0, 0, time.UTC).Add(time.Minute)
}
`,
			want: 0,
		},
		{
			name: "a Sleep on an injected clock is not a wall-clock wait",
			source: `package fixture

type waiter interface{ Sleep(int) }

func parked(clk waiter) { clk.Sleep(1) }
`,
			want: 0,
		},
	}
