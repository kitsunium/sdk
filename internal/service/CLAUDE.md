...
| `codec/` | 15 wire-format codecs over 23 Format names (asn1, baseenc[9], bson, cbor, csv, flatbuffers, form, json, msgpack, ndjson, pem, tlv, toml, xml, yaml), each implementing `core/codec.Codec`; all satisfy `Appender`, most also `StreamingCodec` | `0.3.2.*` … `0.3.40.*` (one PP slot per codec) |
