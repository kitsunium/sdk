...
| `codec/` | 13 wire-format codecs over 21 Format names (asn1, baseenc[9], cbor, csv, flatbuffers, json, msgpack, ndjson, pem, tlv, toml, xml, yaml), each implementing `core/codec.Codec`; all satisfy `Appender`, most also `StreamingCodec` | `0.3.2.*` … `0.3.24.*` (one PP slot per codec) |
