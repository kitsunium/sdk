// Package health_test — the audit that keeps every budget in this package
// testable. The same guard internal/service/lifecycle carries, for the same
// reason and over its own tree.
package health_test
...
// make the check budget untestable in the one way that matters: a timeout
...
// guard on "a slow dependency must not restart the process" with it.
//
// It also protects the cache, whose whole contract is an age: a staleness
// assertion driven by a real clock is a race between the test and the window.
//
...
func TestPackageNeverWaitsOnTheWallClock(t *testing.T) {
	t.Parallel()
	dir := filepath.Join(sdkRoot(t), "internal", "service", "health")
