...
func TestUnmarshal(t *testing.T) {
	...
	tests := []tc{
		...
		{"malformed bytes surface UNMARSHAL_FAILED", []byte{0xff, 0x00}, &bad, "UNMARSHAL_FAILED"},
	}
	runCase := func(t *testing.T, tc tc) {
		t.Helper()
		err := cbor.New().Unmarshal(tc.data, tc.target)
		if tc.wantErr == "" && err != nil {
...
func TestRegisteredViaImport(t *testing.T) {
	...
	tests := []tc{
		{"format registered", func() bool { _, ok := codec.Lookup(codec.Format("cbor")); return ok }},
		{"MIME resolved", func() bool { _, ok := codec.LookupMIME("application/cbor"); return ok }},
		{"extension resolved", func() bool { _, ok := codec.LookupExt(".cbor"); return ok }},
	}
	runCase := func(t *testing.T, tc tc) {
		t.Helper()
		if !tc.check() {
