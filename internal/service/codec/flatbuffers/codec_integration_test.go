...
// TestAllocBudget pins the FlatBuffers passthrough codec's per-call
// allocation ceilings for Marshal, Unmarshal, and (when implemented)
// Append over a small valid buffer (>= the 4-byte root-offset header).
...
func TestAllocBudget(t *testing.T) {
	//: >= 4-byte header so the codec's only validation passes.
	payload := []byte{0x04, 0x00, 0x00, 0x00, 'F', 'B'}
	c := flatbuffers.New()
	...
	if appender, ok := c.(corecodec.Appender); ok {
		dst := make([]byte, 0, 256)
		tests = append(tests, tc{"append", 2, func() {
