...
// Test_jsonDecoder_Decode covers both a success path and a malformed-input
...
func Test_jsonDecoder_Decode(t *testing.T) {
	...
	runCase := func(t *testing.T, tc tc) {
		t.Helper()
		dec := &jsonDecoder{inner: stdjson.NewDecoder(bytes.NewReader(tc.data))}
...
// Test_jsonDecoder_More asserts the sticky EOF latch is idempotent: calling
// More twice in a row on a fresh decoder returns the same value.
func Test_jsonDecoder_More(t *testing.T) {
	t.Parallel()
	type tc struct {
		name string
	}
	...
	runCase := func(t *testing.T, tc tc) {
		t.Helper()
		dec := &jsonDecoder{inner: stdjson.NewDecoder(bytes.NewReader(nil))}
		first := dec.More()
		second := dec.More()
		if first != second {
