...
func TestMarshal(t *testing.T) {
	...
	tests := []tc{
		{"round-trip success", sampleEvent{ID: "x", Severity: 1}, ""},
		{"unsupported input surfaces MARSHAL_FAILED", make(chan int), "MARSHAL_FAILED"},
	}
	runCase := func(t *testing.T, tc tc) {
		t.Helper()
		_, err := json.New().Marshal(tc.in)
...
func TestUnmarshal(t *testing.T) {
	...
	runCase := func(t *testing.T, tc tc) {
		t.Helper()
		err := json.New().Unmarshal(tc.data, tc.target)
		if tc.wantErr == "" && err != nil {
...
func TestNewEncoder(t *testing.T) {
	...
	runCase := func(t *testing.T, tc tc) {
		t.Helper()
		sc, ok := json.New().(codec.StreamingCodec)
		if !ok {
			t.Fatalf("%s: codec does not implement StreamingCodec", tc.name)
		}
		var buf bytes.Buffer
		enc := sc.NewEncoder(&buf)
		if err := enc.Encode(sampleEvent{ID: "x", Severity: 1}); err != nil {
...
func TestRegisteredViaImport(t *testing.T) {
	...
	tests := []tc{
		{"format registered", func() bool { _, ok := codec.Lookup(codec.Format("json")); return ok }},
		{"MIME resolved", func() bool { _, ok := codec.LookupMIME("application/json"); return ok }},
		{"extension resolved", func() bool { _, ok := codec.LookupExt(".json"); return ok }},
	}
	runCase := func(t *testing.T, tc tc) {
		t.Helper()
		if !tc.check() {
