...
| §3.7 Use UTF-8 | **covered on the issue path; normalised, not refused, on the verify path** | An issuer refuses every string it would write that is not valid UTF-8: `iss`/`sub`/`jti`/each `aud` and every private claim's name and bytes at `Issue` (`ISSUE_FAILED`), and the configured `Issuer`/`Type`/`KeyID` at construction (`POLICY_MISCONFIGURED`). Nothing downstream would: `quoteJSONString` passes bytes through, `json.Marshal` REWRITES a bad map key to U+FFFD, and `encoding/json`'s decoder turns `a\xff` and `a\xfe` into one string — so two subjects the caller kept apart would verify as one. A token some OTHER issuer wrote is decoded by `encoding/json`, which replaces an invalid byte rather than refusing it; the base64url decoder is strict, so there is still no second encoding to admit. |
