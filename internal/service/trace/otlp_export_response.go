// Package trace — the collector's answer to an OTLP/HTTP export, and the
...
// jsonNull is the literal a JSON decoder is handed for an explicitly null field;
// it leaves otlpLenientInt64 at its zero, which means "fully accepted".
const jsonNull string = "null"

// otlpExportResponse is ExportTraceServiceResponse, the body of every accepted
// OTLP/HTTP request.
type otlpExportResponse struct {
	PartialSuccess otlpPartialSuccess `json:"partialSuccess"`
}

// otlpPartialSuccess is ExportTracePartialSuccess. errorMessage is declared so
// the shape matches the schema and encoding/json has somewhere to put it; its
// value is never read into an error, because it is unbounded remote-controlled
// text and an errs Field goes straight into structured logs.
type otlpPartialSuccess struct {
	RejectedSpans otlpLenientInt64 `json:"rejectedSpans"`
...
// accepted when decoding". Collectors differ — the reference collector emits the
// string form, some proxies re-serialise it as a number — and a decoder that
// accepted only one of them would silently read every partial success as a full
// one whenever it met the other.
...
// decodeOTLPRejected reports how many spans a 2xx body says were rejected.
//
// An unparseable or empty body is NOT a failure. The request was accepted — that
// is what the 200 said — and the specification's own posture on this exchange is
// that a receiver ignores what it does not recognise; turning a collector's
// malformed response into a lost-data verdict would report a failure that did
// not happen, and would do it on every export.
func decodeOTLPRejected(payload []byte) int64 {
	...
	return int64(decoded.PartialSuccess.RejectedSpans)
}
