...
type OTLPHTTPConfig struct {
	...
	// "http://collector:4318" + OTLPTracesPath, where a reviewer can see the
	// whole address in one string.
	//
	// It is REFUSED at construction when it is not an absolute http(s) URL with
	// a host and a non-root path (OTLPEndpointInvalid): a bare host would POST
	// to "/" and collect 404s forever, which looks exactly like a collector
	// that is up.
	Endpoint string
	...
	// goroutine that is exporting (ADR 0031).
	//
