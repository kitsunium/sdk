...
func Test_decodeRoughtimeMessage(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if _, err := decodeRoughtimeMessage(tt.raw); !errors.Is(err, coreent.ErrCIUnverifiable) {
				t.Errorf("decodeRoughtimeMessage() error = %v, want coreent.ErrCIUnverifiable (%s)", err, tt.reason)
			}
		})
	}
}
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.want < 0 {
				if !errors.Is(err, coreent.ErrCIUnverifiable) {
					t.Errorf("roughtimeHeaderSize() error = %v, want coreent.ErrCIUnverifiable (%s)", err, tt.reason)
				}
				return
			}
			if err != nil {
...
func Test_roughtimeValueBounds(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantStart < 0 {
				if !errors.Is(err, coreent.ErrCIUnverifiable) {
					t.Errorf("roughtimeValueBounds() error = %v, want coreent.ErrCIUnverifiable (%s)", err, tt.reason)
				}
				return
			}
			if err != nil {
