		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				if !errors.Is(err, coreent.ErrCIUnverifiable) {
					t.Errorf("RequestActionsToken() error = %v, want coreent.ErrCIUnverifiable (%s)", err, tt.reason)
				}
				return
			}
			if err != nil {
...
func TestVerifyCI(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			inCI: true,
			roster: &coreent.RosterValue{
				CIAccounts: map[string]coreent.CIEntitlementValue{"133899878": {}},
			},
			wantErr: coreent.ErrCIUnverifiable,
...
func TestRosterValue_CIEntitlementFor(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			accounts:  map[string]coreent.CIEntitlementValue{"42": {ExpiresAt: now.Add(24 * time.Hour)}},
			accountID: "42",
		},
		{
		...
		},
		{
			...
			accounts:  map[string]coreent.CIEntitlementValue{"42": {ExpiresAt: now.Add(-time.Hour)}},
			accountID: "42",
			wantErr:   true,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			roster := &coreent.RosterValue{CIAccounts: tt.accounts}

			_, err := roster.CIEntitlementFor(tt.accountID, now)
			if tt.wantErr {
				if !errors.Is(err, coreent.ErrCINotEntitled) {
					t.Errorf("CIEntitlementFor() error = %v, want coreent.ErrCINotEntitled (%s)", err, tt.reason)
				}
				return
			}
			if err != nil {
