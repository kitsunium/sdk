...
func TestParseBundleRejectsMismatchedHalves(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			older := mustMarshal(t, coreent.RosterValue{IssuedAt: now.Add(-2 * time.Hour), ExpiresAt: now.Add(-time.Hour)})
			newer := mustMarshal(t, coreent.RosterValue{IssuedAt: now.Add(-time.Hour), ExpiresAt: now.Add(time.Hour)})
...
// to look at the network, while coreent.ErrRosterUnsigned claims someone is
...
func TestParseBundleRejectsMalformedInput(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !errors.Is(parseErr, coreent.ErrRosterUnreachable) {
				t.Errorf("ParseBundle(%q) error = %v, want coreent.ErrRosterUnreachable", tt.input, parseErr)
			}
		})
	}
}
...
func TestParseBundle(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			now := time.Now()
			raw := mustMarshal(t, coreent.RosterValue{
				IssuedAt:        now.Add(-time.Hour),
				ExpiresAt:       now.Add(time.Hour),
				Subjects:        map[string]coreent.SubjectValue{sampleUUID: {Fingerprint: "SHA256:x"}},
