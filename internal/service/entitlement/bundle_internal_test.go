...
func TestParseBundleVerifiesBeforeDecoding(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if parseErr.Error() != coreent.ErrRosterUnsigned.Error() {
				t.Errorf("ParseBundle() error = %v, want %v (%s)", parseErr, coreent.ErrRosterUnsigned, tt.reason)
			}
		})
	}
}
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				...
				if !errors.Is(err, coreent.ErrRosterUnreachable) {
					t.Errorf("decodeBundle() error = %v, want coreent.ErrRosterUnreachable (%s)", err, tt.reason)
				}
				return
			}
			if err != nil {
...
func Test_authenticateBundle(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "a bundle signed by anyone else is refused", impostor: true, wantErr: coreent.ErrRosterUnsigned, reason: "the signature is the only thing that makes this instant unforgeable"},
	}
