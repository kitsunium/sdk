...
// Package exec — the int64 syscall.Rlimit constructor. FreeBSD and DragonFly type
// Rlimit.Cur/Max as int64 (their rlim_t is __int64_t), unlike every other Unix
// target. Split into its own build-tagged file so the trampoline
// (trampoline_unix.go, //go:build unix) constructs an Rlimit portably.
package exec
