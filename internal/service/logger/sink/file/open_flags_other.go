...
// Package file — the fallback openFlags for the platforms whose syscall package
// has no O_NOFOLLOW at all: windows, plan9, js/wasm — and wasip1, which has the
// constant but not a kernel behind it (see open_flags_unix.go for why it is
...
// written down in the package CLAUDE.md's platform table rather than smoothed
// over: a table claiming uniform protection would be worse than no table.
//
...
// openFlags is the OpenFile flag set used by New on platforms with no
