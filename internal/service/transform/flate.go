...
func (c flateCompressor) DecompressBounded(dst, src []byte, max int64) (decoded []byte, err error) {
	...
	//: rationale is measurably false: flateDecompressCore(src, 0) reports
	...
	out, tooLarge, derr := flateDecompressCore(dst, src, ceiling)
	...
	if tooLarge {
		//: distinct from FlateFailed: nothing was found malformed. Nothing was
