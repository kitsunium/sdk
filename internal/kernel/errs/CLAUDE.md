...
- **Range-ownership audit (`registry_ownership_external_test.go`, ADR 0035).** The audit above answers *is this code unique*; it structurally cannot see two packages sharing an `MM.LL.PP` with different `SS` bytes — no value ever collides, so range squatting passed green. Two further checks close that:
  ...
  Everything else is IN. A conversion (`CodeX = errs.Code(0x…)`) and an untyped alias of a constant are declarations exactly like the typed form — the collector used to skip any spec without a declared type, so a squat in the conversion spelling was invisible. An `iota` group is evaluated, continuation specs included, and only this package's own Layer-0 block is exempt — iota-numbered, Layer 0, in `internal/kernel/errs`, which `validateDefineArgs` reserves at runtime and `codeRangeOwners` deliberately does not list. And a spec named like a code (`Code` followed by an upper-case word — `Codec` is not one) that fits none of the above — an untyped literal, a variable with no value — is reported as unresolved and FAILS both audits: a spec skipped in silence is a squat nobody sees. `TestAuditPrefixCollectorFailsLoudOnAnUnclassifiableCodeSpec` pins the refusal and the names it must leave alone.
