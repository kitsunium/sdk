...
live under `internal/service/id/` and self-register via a package-level `var` at
import — no `init()`. The canonical external form of every id is its string
rendering, so `Generator.New` returns a `string`.
