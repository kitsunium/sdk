...
type typedNilPlug struct{ transform.Compressor }
...
func TestRegisterRefusesAPlugInTheRegistryCannotStore(t *testing.T) {
	...
	tests := []struct {
		...
		{"a typed nil compressor", func() { transform.Register(typedNil) }, "nil *transform_test.typedNilPlug"},
		{"a non-comparable compressor", func() { transform.Register(uncomparablePlug{tags: []string{"x"}}) }, "transform_test.uncomparablePlug is not comparable"},
	}
