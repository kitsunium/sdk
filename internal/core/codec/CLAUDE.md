...
No format-specific knowledge lives here — concrete codecs live under `internal/service/codec/<format>/` and self-register at package import. `pkg/v1/codec` blank-imports all thirteen of them (18 Format names) and re-exports `Marshal` / `Unmarshal` / `Lookup*` against this package.
