...
type typedNilPlug struct{ metrics.Exporter }
...
func TestRegisterRefusesAPlugInTheRegistryCannotStore(t *testing.T) {
	...
	tests := []struct {
		...
		{"a typed nil exporter", func() { metrics.RegisterExporter(typedNil) }, "nil *metrics_test.typedNilPlug"},
		{"a non-comparable exporter", func() { metrics.RegisterExporter(uncomparablePlug{tags: []string{"x"}}) }, "metrics_test.uncomparablePlug is not comparable"},
	}
