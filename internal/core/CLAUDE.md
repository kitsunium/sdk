...
Domain **interfaces** and immutable domain **value types** for the SDK's domains — **codecs, the logger, and log-transport writers** (ADR 0012). Core describes "what the SDK's domains are" without prescribing how they are realised — every method body belongs in `internal/service/*`, every public alias belongs in `pkg/v1/*`. Plug-in registries (codec, writer) are the deliberate exception: they carry routing state, no domain logic.

## Contents

| Package | Purpose | Code range (ADR 0005/0006/0012) |
|---|---|---|
...
- Grow a **fourth** sibling beside `codec/`, `writer/`, and `logger/` without first widening the layer's purpose statement (the `writer` sibling was admitted by ADR 0012).
