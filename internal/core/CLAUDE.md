...
| `crypto/` | four registries on one `Algorithm` keyspace: the `AEAD` port + redacting `Key` (`Seal`/`Open`), the non-authenticated `Hasher` port (`Sum`/`SumHex`/`NewHash`), the `Signer` port (`Sign`/`Verify`/`GenerateKey`), and the key-separation `Deriver` port (`Subkey`) (ADR 0013) | `0.2.4.*` |
