...
type typedNilPlug struct{ trace.SpanExporter }
...
func TestRegisterRefusesAPlugInTheRegistryCannotStore(t *testing.T) {
	...
	tests := []struct {
		...
		{"a typed nil span exporter", func() { trace.RegisterExporter(typedNil) }, "nil *trace_test.typedNilPlug"},
		{"a non-comparable span exporter", func() { trace.RegisterExporter(uncomparablePlug{tags: []string{"x"}}) }, "trace_test.uncomparablePlug is not comparable"},
	}
