...
| Major | Purpose | State |
|---|---|---|
| `v1/` | Stable public API for logging, error introspection, and codec dispatch (15 codecs covering 23 Format names — incl. base-N family via uniform `codec.Marshal`/`Unmarshal`) | Shipping |
...
- **`pkg/v1/codec` blank-imports all 15 service codec packages** so `import _ "github.com/kitsunium/sdk/pkg/v1/codec"` activates the full registry (asn1, baseenc, bson, cbor, csv, flatbuffers, json, msgpack, multipart, ndjson, pem, tlv, toml, xml, yaml). The baseenc package registers 9 distinct Format names (`base64`, `base64url`, `base32`, `base16`, `hex`, `ascii85`, `base45`, `base58`, `base62`).
