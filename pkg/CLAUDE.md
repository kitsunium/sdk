...
- **No constructors for internal types.** Consumers can introspect SDK errors via `pkg/v1/errs.*Of(err)` accessors (`CodeOf`, `ReasonOf`, `PublicOf`, `PrivateOf`, `HTTPStatusOf`, `ExitCodeOf`, `HasCode`, `HasReason`) but cannot forge `*errs.Error` values. `CodeOf` returns the typed `Code` — octets compose via `code.Layer()` / `code.Major()` / `code.Package()` / `code.Serial()`.
