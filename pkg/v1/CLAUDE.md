...
| `codec/` | Universal codec dispatch: `Marshal` / `Unmarshal` / `NewEncoder` / `NewDecoder` over a `Format` registry; blank-imports 13 service codecs covering 19 Format names — text/binary/base-N reached identically (asn1-der, baseenc family [base64/base64url/base32/base16/hex/ascii85/base45], cbor, csv, flatbuffers, json, msgpack, ndjson, pem, tlv, toml, xml, yaml) | _(no README)_ |
...
- `import _ "github.com/kitsunium/sdk/pkg/v1/codec"` is enough to activate all 13 service codecs (19 Format names incl. base-N family) — registry side-effects driven by blank imports.
