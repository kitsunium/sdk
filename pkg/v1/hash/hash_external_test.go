...
// TestAlgorithmIsDomainDefinedType_V104 pins the V104 fix: hash.Algorithm is a
...
// crypto-family facades shared one identical Go type, so a MAC or signature
// constant fed into a hash call type-checked and only misrouted at runtime. A
...
func TestAlgorithmIsDomainDefinedType_V104(t *testing.T) {
	...
	rt := reflect.TypeFor[hash.Algorithm]()
	tests := []tc{
		//: a defined type reports its declaring package; an alias reports corecrypto's.
		{"package path is this facade", rt.PkgPath(), "github.com/kitsunium/sdk/pkg/v1/hash"},
	...
	for _, c := range tests {
		t.Run(c.name, func(t *testing.T) {
			...
			if c.got != c.want {
				t.Errorf("hash.Algorithm %s=%q want %q (still an alias?)", c.name, c.got, c.want)
			}
		})
	}
}
