// Package yaml_test — a program that imports this package and no other
// codec: it reads YAML through the registry, is refused every other format,
...
import (
	...
	"github.com/kitsunium/sdk/pkg/v1/codec/yaml"
...
func TestItReadsItsFormatThroughTheRegistry(t *testing.T) {
	...
	values, err := config.FSSource(tree, yaml.Format, "app.yaml").Load()
	//: the format this package registers.
	if err != nil {
		t.Fatalf("FSSource(yaml).Load() error = %v", err)
	}
	//: the document's value, decoded.
	if values["name"] != "kit" {
		t.Errorf("FSSource(yaml).Load() = %v, want name=kit", values)
	}
...
func TestItRegistersItsFormatAlone(t *testing.T) {
	...
	if got := corecodec.Available(); !slices.Equal(got, []corecodec.Format{yaml.Format}) {
		t.Fatalf("registered formats = %v, want only %q", got, yaml.Format)
	}
}
...
func TestGoListDepsNamesNoOtherCodec(t *testing.T) {
	...
	output, err := exec.Command(goTool, "list", "-deps", "github.com/kitsunium/sdk/pkg/v1/codec/yaml").Output()
	...
	for line := range strings.FieldsSeq(string(output)) {
		...
		if (strings.HasPrefix(line, "github.com/kitsunium/sdk/internal/service/codec/") && line != "github.com/kitsunium/sdk/internal/service/codec/yaml") ||
