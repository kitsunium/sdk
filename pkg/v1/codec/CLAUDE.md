...
codec.go      — Format alias, 23 Format constants, Marshal/Unmarshal/NewEncoder/NewDecoder,
                ...
                blank imports for asn1|baseenc|bson|cbor|csv|flatbuffers|form|json|msgpack|ndjson|pem|tlv|toml|xml|yaml
...
- **`Format` is the public dispatch key.** It's `type Format = corecodec.Format` — a string alias, but the 23 named constants (`JSON`, `NDJSON`, `XML`, `CSV`, `Form`, `ASN1DER`, `PEM`, `YAML`, `TOML`, `CBOR`, `MsgPack`, `TLV`, `FlatBuffers`, `Base64`, `Base64URL`, `Base32`, `Base16`, `Hex`, `ASCII85`, `Base45`, `Base58`, `Base62`, `BSON`) are the contract. Their string values are frozen post-v1.0.0.
