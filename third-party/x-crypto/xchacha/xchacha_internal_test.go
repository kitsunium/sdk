...
func Test_xChaCha_Algorithm(t *testing.T) {
	t.Parallel()
	type tc struct {
		name string
	...
	runCase := func(t *testing.T, c tc) {
		...
		if got := (xChaCha{}).Algorithm(); got != c.want {
...
func Test_xChaCha_ID(t *testing.T) {
	t.Parallel()
	type tc struct {
		name string
		want byte
	...
	runCase := func(t *testing.T, c tc) {
		t.Helper()
		//: the wire id must be the frozen 0x02 used for Open dispatch.
		if got := (xChaCha{}).ID(); got != c.want {
...
func Test_newAEAD(t *testing.T) {
	t.Parallel()
	type tc struct {
		name string
	}
	tests := []tc{{"a 256-bit key builds a usable XChaCha20-Poly1305 cipher"}}
	runCase := func(t *testing.T, _ tc) {
		t.Helper()
		aead, err := newAEAD(mustKey(t, 0x4).Bytes())
		//: a valid key must yield a non-nil cipher with the 24-byte nonce.
		if err != nil || aead == nil || aead.NonceSize() != nonceLen {
			t.Errorf("newAEAD=(%v,%v) nonceSize check failed", aead, err)
		}
	}
	for _, c := range tests {
		t.Run(c.name, func(t *testing.T) {
...
func Test_xChaCha_Seal(t *testing.T) {
	t.Parallel()
	type tc struct {
		name      string
		plaintext []byte
	...
	runCase := func(t *testing.T, c tc) {
		t.Helper()
		key := mustKey(t, 0x5)
		box, err := xChaCha{}.Seal(key, c.plaintext, c.aad)
		...
		box2, serr := (xChaCha{}).Seal(key, c.plaintext, c.aad)
...
func Test_xChaCha_Open(t *testing.T) {
	t.Parallel()
	type tc struct {
		name   string
	...
	runCase := func(t *testing.T, c tc) {
		...
		box, err := xChaCha{}.Seal(key, []byte("secret payload"), []byte("ctx"))
		if err != nil {
			t.Fatalf("%s: Seal: %v", c.name, err)
		}
		mbox, openKey, openAAD := c.mutate(box, key, other)
		_, oerr := xChaCha{}.Open(openKey, mbox, openAAD)
