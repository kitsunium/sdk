...
func (extProvider) Credentials(_ context.Context) (writer.CredentialValue, error) {
	//: fixed material; the external test never performs a real delivery.
...
func TestOpenViaRegistry(t *testing.T) {
	...
	tests := []tc{
		{"valid config resolves and builds a sink", writer.CloudWatchConfig{Group: "g", Stream: "s", Region: "eu-west-3", Credentials: extProvider{}}, false},
		{"wrong config type is rejected", writer.FileConfig{Path: "/x"}, true},
	}
	runCase := func(t *testing.T, c tc) {
		t.Helper()
		sink, err := writer.Open("cloudwatch", c.cfg)
