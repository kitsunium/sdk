...
func Test_Service_currentRoster(t *testing.T) {
	...
	raw, err := json.Marshal(RosterValue{
		IssuedAt:  now.Add(-time.Hour),
		ExpiresAt: now.Add(RosterLifetime - time.Hour),
		Subjects:  map[string]SubjectValue{sampleSubject: {Fingerprint: "SHA256:x"}},
	})
	if err != nil {
	...
	tests := []struct {
		...
		{name: "an origin that stopped publishing yields no decision", failAll: true, wantErr: ErrRosterUnreachable},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			svc := NewServiceWithGetter(stubRoundTripper{bundle: bundle, fail: tt.failAll}, t.TempDir(), pub, &testProduct)
...
func Test_Service_matchSubject(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "an unlisted subject is revoked", listed: false, wantErr: ErrRevoked},
		{name: "a listed subject with no local key is unlicensed", listed: true, wantErr: ErrNoLicense},
		{name: "a listed subject past its own term is expired", listed: true, expiry: now.Add(-time.Second), wantErr: ErrLicenseExpired},
		{name: "a listed subject before its own term is unlicensed, not expired", listed: true, expiry: now.Add(time.Second), wantErr: ErrNoLicense},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.listed {
				subjects[sampleSubject] = SubjectValue{Fingerprint: "SHA256:whatever", ExpiresAt: tt.expiry}
			}
			svc := NewServiceWithGetter(stubRoundTripper{}, t.TempDir(), nil, &testProduct)
			_, err := svc.matchSubject(&RosterValue{Subjects: subjects}, sampleSubject, now)
...
func Test_Service_fetch(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "a 404 yields no decision", status: http.StatusNotFound, wantErr: ErrRosterUnreachable},
		{name: "a 500 yields no decision", status: http.StatusInternalServerError, wantErr: ErrRosterUnreachable},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			svc := NewServiceWithGetter(statusGetter{status: tt.status}, t.TempDir(), nil, &testProduct)
...
func signBundle(t *testing.T, priv ed25519.PrivateKey, roster RosterValue) []byte {
	t.Helper()
...
func Test_Service_rosterFrom(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			now := time.Now()
			getter := &bundleGetter{bundle: signBundle(t, signingKey, RosterValue{
				IssuedAt:  now.Add(-time.Hour + tt.window),
				ExpiresAt: now.Add(time.Hour + tt.window),
				Subjects:  map[string]SubjectValue{sampleSubject: {Fingerprint: "SHA256:whatever"}},
			})}
			svc := NewServiceWithGetter(getter, t.TempDir(), vendorPub, &testProduct)

			origin := OriginValue{Name: "solo", BundleURL: "https://x/roster.signed.json"}
...
func Test_Service_rosterFrom_PropagatesFailures(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			svc := NewServiceWithGetter(&stubStatusGetter{status: tt.status, body: tt.body}, t.TempDir(), vendorPub, &testProduct)
			origin := OriginValue{Name: "solo", BundleURL: "https://x/roster.signed.json"}

			_, rosterErr := svc.rosterFrom(origin, time.Now())
			if !errors.Is(rosterErr, ErrRosterUnreachable) {
				t.Errorf("rosterFrom() error = %v, want ErrRosterUnreachable (%s)", rosterErr, tt.reason)
			}
		})
	}
}
...
func Test_Service_authorise(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			roster := &RosterValue{IssuedAt: now.Add(-time.Minute), ExpiresAt: now.Add(time.Hour), CIRelaxed: tt.relaxed}

			svc := NewServiceWithGetter(stubRoundTripper{fail: true}, t.TempDir(), nil, &testProduct)
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				...
				if !errors.Is(err, ErrRosterUnreachable) {
					t.Errorf("readBounded() error = %v, want ErrRosterUnreachable (%s)", err, tt.reason)
				}
				return
			}
			if err != nil {
