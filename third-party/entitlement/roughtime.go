...
func QueryRoughtime(server RoughtimeServerValue) (midpoint time.Time, radius time.Duration, err error) {
	...
	if len(server.PublicKey) != ed25519.PublicKeySize {
		...
		return time.Time{}, 0, fmt.Errorf("%w: roughtime server %q has no usable key", ErrCIUnverifiable, server.Name)
	}
	...
	if _, randErr := rand.Read(nonce[:]); randErr != nil {
		...
		return time.Time{}, 0, fmt.Errorf("%w: drawing roughtime nonce: %w", ErrCIUnverifiable, randErr)
	}
...
func roughtimeExchange(address string, request []byte) (response []byte, err error) {
	...
	if dialErr != nil {
		//: Report it as unverifiable.
		return nil, fmt.Errorf("%w: dialling roughtime %s: %w", ErrCIUnverifiable, address, dialErr)
	}
	...
	if deadlineErr := conn.SetDeadline(time.Now().Add(roughtimeTimeout)); deadlineErr != nil {
		//: Report it as unverifiable.
		return nil, fmt.Errorf("%w: roughtime deadline: %w", ErrCIUnverifiable, deadlineErr)
	}
	//: A request we could not send earns no answer.
	if _, writeErr := conn.Write(request); writeErr != nil {
		//: Report it as unverifiable.
		return nil, fmt.Errorf("%w: roughtime request: %w", ErrCIUnverifiable, writeErr)
	}
	...
	if readErr != nil {
		//: Report it as unverifiable.
		return nil, fmt.Errorf("%w: roughtime response: %w", ErrCIUnverifiable, readErr)
	}
