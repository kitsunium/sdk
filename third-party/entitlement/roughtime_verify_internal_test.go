		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				...
				if !errors.Is(err, ErrCIUnverifiable) {
					t.Errorf("verifyRoughtimeResponse() error = %v, want ErrCIUnverifiable (%s)", err, tt.reason)
				}
				return
			}
			if err != nil {
...
func Test_verifyRoughtimeMerkle(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				if !errors.Is(err, ErrCIUnverifiable) {
					t.Errorf("verifyRoughtimeMerkle() error = %v, want ErrCIUnverifiable (%s)", err, tt.reason)
				}
				return
			}
			if err != nil {
