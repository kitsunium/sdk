		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				if !errors.Is(err, ErrCIUnverifiable) {
					t.Errorf("checkTokenURL(%q) error = %v, want ErrCIUnverifiable (%s)", tt.raw, err, tt.reason)
				}
				return
			}
			if err != nil {
...
func Test_tokenResponseBody(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				if !errors.Is(err, ErrCIUnverifiable) {
					t.Errorf("tokenResponseBody() error = %v, want ErrCIUnverifiable (%s)", err, tt.reason)
				}
				return
			}
			if err != nil {
...
func Test_fetchToken(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				if !errors.Is(err, ErrCIUnverifiable) {
					t.Errorf("fetchToken() error = %v, want ErrCIUnverifiable (%s)", err, tt.reason)
				}
				return
			}
			if err != nil {
