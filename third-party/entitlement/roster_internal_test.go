...
func TestRosterLifetime(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.atMost && RosterLifetime > tt.bound {
				t.Errorf("RosterLifetime = %v, want at most %v (%s)", RosterLifetime, tt.bound, tt.reason)
			}
			if !tt.atMost && RosterLifetime < tt.bound {
				t.Errorf("RosterLifetime = %v, want at least %v (%s)", RosterLifetime, tt.bound, tt.reason)
			}
		})
	}
}
...
func Test_authenticateRoster(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "a vendor key of the wrong length is refused", shortKey: true, wantErr: ErrRosterUnsigned, reason: "ed25519.Verify panics on a short slice; a broken build must not take the process down"},
		{name: "a payload signed by anyone else is refused", impostor: true, wantErr: ErrRosterUnsigned, reason: "the signature runs before the JSON parser, which is the whole ordering"},
