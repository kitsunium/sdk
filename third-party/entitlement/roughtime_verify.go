...
func checkRoughtimeBinding(stated roughtimeStatement, delegated roughtimeDelegation, fields map[uint32][]byte, nonce []byte) error {
	...
	if stated.midpoint < delegated.notBefore || stated.midpoint > delegated.notAfter {
		//: Report the refusal.
		return fmt.Errorf("%w: roughtime midpoint outside its delegation window", ErrCIUnverifiable)
	}
...
func unframeRoughtime(packet []byte) (message []byte, err error) {
	...
	if len(packet) < header || string(packet[:len(roughtimeFrameMagic)]) != roughtimeFrameMagic {
		...
		return nil, fmt.Errorf("%w: roughtime packet is not framed", ErrCIUnverifiable)
	}
	...
	if int(stated) > len(packet)-header {
		//: Report the malformed answer.
		return nil, fmt.Errorf("%w: roughtime frame claims %d bytes, got %d", ErrCIUnverifiable, stated, len(packet)-header)
	}
...
func verifyRoughtimeCert(cert []byte, longTerm ed25519.PublicKey) (delegation roughtimeDelegation, err error) {
	...
	if len(key) != ed25519.PublicKeySize {
		//: Report the malformed answer.
		return roughtimeDelegation{}, fmt.Errorf("%w: roughtime delegated key is %d bytes", ErrCIUnverifiable, len(key))
	}
	...
	if beforeErr != nil || afterErr != nil {
		//: Report the malformed answer.
		return roughtimeDelegation{}, fmt.Errorf("%w: roughtime delegation window is unreadable", ErrCIUnverifiable)
	}
...
func checkRoughtimeSignature(key ed25519.PublicKey, context string, signed, signature []byte) error {
	...
	if len(signature) != ed25519.SignatureSize {
		//: Report the malformed answer.
		return fmt.Errorf("%w: signature is %d bytes, want %d", ErrCIUnverifiable, len(signature), ed25519.SignatureSize)
	}
...
func readRoughtimeSrep(srep []byte) (stated roughtimeStatement, err error) {
	...
	if len(root) != roughtimeHashSize {
		//: Report the malformed answer.
		return roughtimeStatement{}, fmt.Errorf("%w: roughtime root is %d bytes, want %d", ErrCIUnverifiable, len(root), roughtimeHashSize)
	}
...
func verifyRoughtimeMerkle(fields map[uint32][]byte, nonce, root []byte) error {
	...
	if len(path)%roughtimeHashSize != 0 || len(path)/roughtimeHashSize > roughtimeMaxPathDepth {
		//: Report the malformed answer.
		return fmt.Errorf("%w: roughtime path is %d bytes", ErrCIUnverifiable, len(path))
	}
	...
	if !bytes.Equal(hash, root) {
		//: Report the refusal.
		return fmt.Errorf("%w: roughtime path does not reach the signed root", ErrCIUnverifiable)
	}
