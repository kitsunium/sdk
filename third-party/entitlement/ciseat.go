...
func (s *Service) ciSeat(roster *RosterValue, now time.Time) (grant GrantValue, err error) {
	//: Nothing to try outside Actions, and no reason to fetch a key set.
	if !InCI() {
		//: Report it as unprovable; the caller falls through.
		return GrantValue{}, fmt.Errorf("%w: not running in GitHub Actions", ErrCIUnverifiable)
	}
	...
	if len(roster.CIAccounts) == 0 {
		...
		return GrantValue{}, fmt.Errorf("%w: roster grants no CI entitlement", ErrCINotEntitled)
	}
	...
	if entitlementErr != nil {
		...
		return GrantValue{}, entitlementErr
	}
	...
	return GrantValue{
		...
		VerifiedAt: now,
		NotAfter:   grantDeadline(now, roster.ExpiresAt, entitlement.ExpiresAt),
	}, nil
}
...
// subtle part: s.fetch speaks in ErrRosterUnreachable because its other caller
...
// licenseExitCode and licenseAdvice both match ErrRosterUnreachable ahead of
...
func (s *Service) publishedJWKS() (keys map[string]*rsa.PublicKey, err error) {
	...
	if fetchErr != nil {
		...
		return nil, fmt.Errorf("%w: fetching the key set: %s", ErrCIUnverifiable, fetchErr.Error())
	}
...
func ciRefusalIsFinal(roster *RosterValue) bool {
...
// ErrRosterUnreachable inside ErrCIUnverifiable. licenseExitCode matches
// ErrRosterUnreachable BEFORE ErrLicenseExpired, and licenseAdvice does the
