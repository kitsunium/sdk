...
func ParseRoster(raw, sig []byte, vendor ed25519.PublicKey, now time.Time) (roster *RosterValue, err error) {
	...
	if decoded.ExpiresAt.Sub(decoded.IssuedAt) > RosterLifetime {
		//: Refuse an over-wide window whoever signed it.
		return nil, fmt.Errorf("%w: window exceeds %s", ErrRosterStale, RosterLifetime)
	}
...
func authenticateRoster(raw, sig []byte, vendor ed25519.PublicKey) (roster *RosterValue, err error) {
	...
	if len(vendor) != ed25519.PublicKeySize {
		...
		return nil, fmt.Errorf("%w: malformed vendor key", ErrRosterUnsigned)
	}
	...
	if !ed25519.Verify(vendor, raw, sig) {
		...
		return nil, ErrRosterUnsigned
	}

	var decoded RosterValue
