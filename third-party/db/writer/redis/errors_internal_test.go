...
func Test_wrapClientInit(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !errs.HasCode(err, CodeRedisClientInitFailed) {
...
func Test_wrapAdd(t *testing.T) {
	t.Parallel()
	tests := []struct {
		name  string
		cause error
	}{
		{"nil cause carries the add code", nil},
		{"non-nil cause stays Is-matchable", errInnerBoom},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			err := wrapAdd(tc.cause, 5)
			//: every wrap must carry the add code.
			if !errs.HasCode(err, CodeRedisXAddFailed) {
				t.Errorf("%s: err=%v want add", tc.name, err)
			}
			//: a non-nil cause must remain reachable via errors.Is.
			if tc.cause != nil && !errors.Is(err, tc.cause) {
				t.Errorf("%s: errors.Is lost the cause", tc.name)
