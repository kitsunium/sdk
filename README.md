...
| [`pkg/v1/codec`](./pkg/v1/codec) | Universal codec dispatch over a `Format` registry. 24 formats covered by a single `Marshal` / `Unmarshal` / `NewEncoder` / `NewDecoder` API — `asn1-der`, `bson`, `cbor`, `csv`, `flatbuffers`, `form`, `json`, `msgpack`, `multipart`, `ndjson`, `pem`, `tlv`, `toml`, `xml`, `yaml` + 9 base-N encodings (`base16`, `base32`, `base45`, `base58`, `base62`, `base64`, `base64url`, `hex`, `ascii85`). |
