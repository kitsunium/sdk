...
- ADR 0027 — observability domain (`metrics`): 9th core sibling, Counter/Gauge/Histogram + `Meter` + `Exporter` registry (writer-registry model); in-memory meter + stdlib text exporter; error block `0.2.9.*`; **labels shipped in place** — name + label set = one series, bounded per name with a visible aggregated overflow series (`0.2.9.4` INVALID_LABEL); Prometheus/OTLP deferred — `docs/adr/0027-sdk-metrics-domain.md`
