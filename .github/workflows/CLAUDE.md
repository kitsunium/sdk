...
10. **Lint gate** — installs `gofumpt` and `ktn-linter` at PINNED versions, then `make lint-check`. Those are the three checks of `make lint` that no lane ran until #236 — `ktn-linter`, `gofumpt -l` and `make guard`; the other five are steps 2, 6, 7, 8 and 9 above, so this step adds a CLASS of analysis rather than repeating one. It is in THIS job, not in `shell-gates`, because `bazel` and `post-commit` are the only required checks on `main` and a sibling job would report without blocking. `kodflow/ktn-linter` is a private repository: the install goes through `gh release download` (a `curl` on the release URL answers 404 either way — the signed redirect drops the Authorization header) and needs a `KTN_LINTER_TOKEN` secret, since `secrets.GITHUB_TOKEN` is scoped to this repository. The step asserts the installed version matches the pin, because 1.9.11 exits 0 with "No issues found" on a tree 1.11.2 rejects.
11. `bazel coverage --combined_report=lcov //...` → uploaded as `coverage-${{ github.run_number }}` artifact (per-run unique name so concurrent runs don't dedupe, post-audit finding #28). Note coverage runs under the default (race-on) config, so it does **not** reflect the step-5 tests.
...
These four, plus `make lint-check` in the `bazel` job, are the only `make`
invocations in this workflow, and that is deliberate: `ci-gates-check` asserts
the Makefile↔CI link by target NAME, which only works if CI goes through the
target. `lint-check` is the one gate that is NOT in this job, because it needs a
Go toolchain and two downloaded binaries — the two things `shell-gates` exists
to do without. Adding a gate means editing `GATES` **and** this
