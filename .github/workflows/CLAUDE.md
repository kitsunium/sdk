...
| `sdk-release.yml` | `workflow_run` after `SDK CI (Bazel)` success on `main`, plus manual `workflow_dispatch` | Impact-driven patch tags `pkg/<major>/vX.Y.Z` (see ADR 0007). Reads majors from `scripts/release/compute-bumps.sh` and pushes via `scripts/release/cut-tags.sh`. First release is held unless dispatched manually (`--allow-bootstrap`, ADR 0009). The `Release-bump` trailer that sizes a minor is read over the whole range since the last release, not from the checked-out HEAD — a cancelled CI run yields no release, so the trailer-bearing commit is often not HEAD by the time one fires (ADR 0085). |
