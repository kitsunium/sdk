...
8. **Domain-doc invariant** — `scripts/pre-commit/check-domain-docs.sh` fails the build when the root `CLAUDE.md`'s architecture tree no longer names exactly the directories under `internal/core`, when a domain is described twice in the Purpose paragraph, or when two Purpose paragraphs coexist. All three are what a union merge of that file leaves behind, and all three had actually happened.
